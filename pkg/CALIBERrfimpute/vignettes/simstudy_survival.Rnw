...
coximpute <- function(imputed_datasets){
	...
	if ('lo 95' %in% colnames(out)){
		# older version of mice
		out <- cbind(out, data.frame(cover = 
			kLogHR >= out[, 'lo 95'] & kLogHR <= out[, 'hi 95']))
	} else if ('estimate' %in% colnames(out)){
		...
		CI_lo <- out$estimate + qnorm(0.025) * out$std.error
		CI_hi <- out$estimate + qnorm(0.975) * out$std.error
		out <- cbind(out, data.frame(cover =
			kLogHR >= CI_lo & kLogHR <= CI_hi))
	} else {
